...
import { AlertCircle, Loader2, Webcam, WifiOff } from "lucide-react";
    ...
    const initClient = async () => {
      try {
        ...
        if (error?.message?.includes("JoinBackstage")) {
          setErrorMessage(
            "The host is preparing the webinar. Please wait for the broadcast to begin!",
          );
        } else {
          setErrorMessage(
            ...
            <>
              <div className="relative mx-auto w-24 h-24 mb-6">
                ...
                <div className="absolute inset-3 rounded-full bg-card flex items-center justify-center">
                  <Webcam className="h-10 w-10 text-accent-primary animate-pulse" />
                </div>
              </div>
