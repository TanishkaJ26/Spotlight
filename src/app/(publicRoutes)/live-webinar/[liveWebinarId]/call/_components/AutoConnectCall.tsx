...
const AutoConnectCall = ({
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-gradient-to-br from-background via-background to-accent-primary/5 relative overflow-hidden">
        ...
        <div className="w-full h-full max-w-6xl mx-auto flex flex-col md:grid md:grid-cols-2 gap-4 md:gap-8">
          ...
          <div className="relative flex-1 md:flex-none md:aspect-video bg-card/30 backdrop-blur-xl rounded-3xl overflow-hidden border border-white/5 shadow-2xl flex items-center justify-center group transition-all duration-500 hover:border-white/10">
            ...
            <div className="relative">
              ...
              <div
                className={cn(
                  "flex justify-center items-center rounded-full overflow-hidden border-4 p-5 md:p-8 transition-all duration-500",
                  assistantIsSpeaking
                    ? "border-accent-primary bg-accent-primary/10 shadow-[0_0_40px_rgba(var(--accent-primary),0.3)] scale-110"
                    : "border-white/10 bg-black/20"
                )}
              >
                <Bot className={cn("w-16 h-16 md:w-20 md:h-20 transition-all duration-500", assistantIsSpeaking ? "text-accent-primary" : "text-white/40")} />
              </div>
            </div>
          </div>
          ...
          <div className="relative flex-1 md:flex-none md:aspect-video bg-card/30 backdrop-blur-xl rounded-3xl overflow-hidden border border-white/5 shadow-2xl flex items-center justify-center group transition-all duration-500 hover:border-white/10">
            ...
            {/* Timer Label */}
            <div className="absolute top-4 right-4 bg-black/40 backdrop-blur-md text-white/90 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium flex items-center gap-2 border border-white/5 shadow-lg">
              <Clock className={cn("h-3 w-3 md:h-4 md:w-4", timeRemaining < 30 ? "text-destructive" : "text-muted-foreground")} />
            ...
            <div className="relative">
              ...
              >
                <Avatar className="w-24 h-24 md:w-32 md:h-32">
                  ...
                  <AvatarFallback className="text-4xl md:text-5xl font-bold uppercase text-white bg-secondary/30">
                    {userName.split("")?.[0]}
                  </AvatarFallback>
                </Avatar>
              </div>
              ...
              {isMicMuted && (
                <div className="absolute -bottom-2 -right-2 md:-bottom-3 md:-right-3 bg-destructive text-white p-2 md:p-3 rounded-full shadow-lg border-4 border-background">
      ...
      <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-20 w-full max-w-md px-4 md:px-0">
        <div className="bg-black/60 backdrop-blur-2xl border border-white/10 p-2 md:p-3 rounded-full flex items-center justify-center gap-3 md:gap-4 shadow-2xl mx-auto w-fit md:w-full">
          
          <Button
            onClick={toggleMicMute}
            variant="ghost"
            className={cn(
              "h-12 w-12 md:h-14 md:w-14 rounded-full transition-all duration-300 shrink-0",
              isMicMuted
          ...
          <Button
            onClick={stopCall}
            variant="destructive"
            className="h-12 w-12 md:h-14 md:w-14 rounded-full hover:bg-destructive/90 transition-all duration-300 shadow-[0_0_20px_rgba(239,68,68,0.4)] shrink-0"
            title="End Call"
            disabled={callStatus !== CallStatus.ACTIVE}
          >
            <PhoneOffIcon className="h-5 w-5 md:h-6 md:w-6" />
          </Button>

          <div className="w-[1px] h-6 md:h-8 bg-white/20 mx-1 md:mx-2 shrink-0" />

          <Dialog>
            <DialogTrigger asChild>
              <Button className="h-10 md:h-12 px-5 md:px-6 rounded-full bg-accent-primary hover:bg-accent-primary/90 text-white text-sm md:text-base font-semibold transition-all duration-300 shadow-[0_0_20px_rgba(var(--accent-primary),0.4)] whitespace-nowrap">
                Buy Now
              </Button>
            </DialogTrigger>
